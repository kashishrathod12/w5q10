...
3. **Auth accepts any nonempty bearer token** as a principal identifier —
   correct for this grading setup (the grader supplies its own tokens to
   probe isolation, so an allowlist would reject them), but before this
   touches anything real, swap it for a real identity provider (JWT/OAuth)
   that verifies the token was actually issued by you.
